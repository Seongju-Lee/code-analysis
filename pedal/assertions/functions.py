...
class missing_function(AssertionFeedback):
    ...
    title = "함수 없음"
    message_template = "{name_message}이란 함수를 찾을 수 없습니다."
...
class duplicate_function_definition(AssertionFeedback):
    ...
    title = "중복 함수 정의"
    message_template = ("{name_message}이란 함수가 {lines_message}번 줄에 여러번 정의되었습니다.")
...
class too_few_parameters(AssertionFeedback):
    """ Too Few Parameters """
    title = "너무 적은 매개변수"
    message_template = ("{name_message}라는 함수에 너무 적은 ({found}) 매개 변수가 있습니다 ({expected}).")
